...
# -*- coding: utf-8 -*-
"""
Created on Sat Sep 22 21:07:30 2018
...
#    self.save(args.dataset + '.model')
#
#    with open(args.dataset + '.params', 'w') as fout:
#        json.dump(self, f_out, indent=2)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='PyTorch implementation of VDPWI')
    parser.add_argument('model_outfile', help='file to save final model')
    parser.add_argument('--dataset', help='name of directory that contains dataset', default='eng_fre')
    ...
    # VDPWI args
    parser.add_argument('--classifier', type=str, default='vdpwi', choices=['vdpwi', 'resnet','vdpwi_alt'])
    ...
    #SAVE the model
    with open(args.dataset + '.model', 'wb') as fout:
    ...
    ALL_PAIRS = other_evaluate('test', dataset_cls, model, embedding, test_loader, args.batch_size, args.device)
    with open(args.dataset + "_classify.txt", 'w') as f_out:
