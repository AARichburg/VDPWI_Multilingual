...
# -*- coding: utf-8 -*-
"""
Created on Sat Sep 22 20:58:53 2018
...
class VDPWIConvNetm(nn.Module):
    def __init__(self, config):
        super(VDPWIConvNet,self).__init__()
...
class VDPWIModel(nn.Module):
    def __init__(self, dim, config):
        super(VDPWIModel,self).__init__()
        self.arch = 'vdpwi'
        ...
        elif config['classifier'] == 'vdpwi_alt':
            self.classifier_net == VDPWIConvNet_ALT(config)
...
#        log_prob = self.classifier_net(sim_cube)
        log_prob = self.classifier_net(focus_cube)
        soft_max_ = F.softmax(log_prob, dim=1)
        #return soft_max_
        return log_prob
